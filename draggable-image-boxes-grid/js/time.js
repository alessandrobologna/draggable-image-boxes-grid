   ...
   if (typeof window.Onebot.count === 'undefined') {
        ...
        // default opts
        async : true,
        serviceUrl: 'http://cdn.api.onebot.timeinc.com/stats/com/time.count.json?s=',
        ...
        get : function (service, callback, urlArr, opts) {
            ...
            // Removed duplicates from Array.prototype.unique
            _self.urlArr = _self.urlArr.unique();
            var urls = []; 
            var badUrls = [];
        ...
        parseUrl : function (url) {
            ...
            if (!strArr[2] || strArr[2].indexOf(this.getDomainBase()) == -1) { // too short or not part of domain
                return null;
            }
        ...
        getDomainBase : function () {
            // Lazy load once
            if (!this.domainBase) {
                var paths = "//cdn.api.onebot.timeinc.com/stats/com/time".split('/').reverse();
                var i =0;
    ...
    if (typeof window.Onebot.query === 'undefined') {
        window.Onebot.query = {
            serviceUrl : '//cdn.api.onebot.timeinc.com/stats/com/time.query.',
